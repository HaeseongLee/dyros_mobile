...
#define XR_Iv (  2.1 )       // kg m2  --> these two values must be modifed later!!!!
...
class Caster
{
       public:
              ...
              void Fill_LM(double const _u,  // phi dot
                           double const _v,  // rho  dot
                           double const _w); // sigma dot
              ...
              double a, c, s; // phi, cos(phi), sin(phi)
              double u, v, w; // phi dot, rho dot, sigma dot

              double ang; // to compute enc_offset
              long enc_offset; // FOR CALIBRATION OF EXACT HOME SENSOR POSITION

       private:
              // dynamics in the JT-SPACE
              void Fill_A();
              void Fill_CC();
              
              // dynamics in the OP-SPACE
              void Fill_L_Pk();
              void Fill_Mu_Pk();
              ...
              Matrix3d Ce; // wheel constraint matrix, J_inverse
              Matrix3d Jdot;
              Matrix3d A; // JT-SPACE mass matrix
              Vector3d CC; // JT-SPACE coriollis & centrifugal
